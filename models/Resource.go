...
// ResourceTreeGridByUserId 根据用户获取所有权管理的资源列表， 并整理成treegrid格式
func ResourceTreeGridByUserId(backuserid, maxrtype int) []*Resource {
	...
	if user.IsSuper == true {
		...
		count, _ := o.Raw(sql, maxrtype).QueryRows(&list)
		fmt.Println(count)
	} else {
		//联查多张表，找出某用户有权管理的
		...
		count, _ := o.Raw(sql, backuserid, maxrtype).QueryRows(&list)
		fmt.Println(count)
	}
